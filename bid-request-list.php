      ...
      <section class="table-container">
        ...
        <?php              
            ...
            <tr>
                ...
                <td class="text-start"><div class="userthumb"><img class="rounded-circle" src="assets/images/building.png"/></div> <span><?php echo $request['societyName']; ?></span></td>
                <td><span><?php echo $request['propertyType']; ?></span></td>
                <td style="white-space: pre-wrap;"><span><?php echo $request['location']; ?></span></td>
                <td> <span><?php $propertyDetails=unserialize($request['property']);
                                                foreach($propertyDetails as $pkey=>$Detail){
                                                      echo $pkey.": ".$Detail."<br/>"; 
                                                }
                                                 
                                          ?></span> </td>
                  <td><span><?php echo date("d/m/Y", strtotime($request['datetime'])); ?></span></td>
                
                <td><?php echo $request['status'] == 1 ? '<span class="txt-approved" ><i class="far fa-dot-circle   me-2"></i>Approved</span>' : '<span class="txt-pending"><i class="far fa-dot-circle  me-2"></i>Pending</span>'; ?></td>
                <td>
                <?php if($request['status'] == 0){ ?>
                                            <button type="button" data-id="<?php echo $request['recid']; ?>" class="btn background-one rounded-pill text-white text-nowrap">Invite Bids</button>
                                            <?php } ?></td>
                           ...
                           <div class="row">
                               <table id="inviteList" class="inviteListWrapper invitesList table table-striped  nowrap">
                                   ...
                                   <tbody>
                                      ...
                                      $bidrequests = getbidrequests($pdo); 
                                      foreach($bidrequests as $request){ ?>
                                       <tr>
                                          <td></td>
                                          <td></?php echo $request['societyName']; ?></td>
                                          <td></?php echo $request['propertyType']; ?></td>
                                          <td style="white-space: pre-wrap;"></?php echo $request['location']; ?></td>
                                          <td>
                                          </?php $propertyDetails=unserialize($request['property']);
                                                foreach($propertyDetails as $pkey=>$Detail){
                                                      echo $pkey.": ".$Detail."<br/>"; 
                                                }
                                          ...
                                          <td></?php echo date("d/m/Y", strtotime($request['datetime'])); ?></td>
                                          <td></?php echo $request['status'] == 1 ? 'Approved' : 'Pending'; ?></td>
                                          <td>
                                             </?php if($request['status'] == 0){ ?>
                                            <button type="button" data-id="</?php echo $request['recid']; ?>" class="btn themeBg invitebidbutton">Invite Bids</button>
                                            </?php } ?>
                                          </td>
                                       </tr>
                                      </?php }
